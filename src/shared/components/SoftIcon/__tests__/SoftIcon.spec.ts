...
describe('SoftIcon.vue', () => {
    it('renders an icon with size class', async () => {
        const render = customRenderer(SoftIcon).withProps({
            name: 'pencil',
            label: 'Edit',
        });
        ...
        const icon = container.querySelector('.soft-icon--md');
    ...
    it('applies size class when size prop is set', async () => {
        ...
        const icon = container.querySelector('.soft-icon--sm');
        expect(icon).toBeInTheDocument();
    });
    ...
    it('renders different icon by name', async () => {
        ...
        const svg = container.querySelector('svg');
        expect(svg).toBeInTheDocument();
        expect(svg).toHaveClass('soft-icon--md');
    });
});
